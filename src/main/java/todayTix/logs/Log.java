...
import todayTix.utils.GeneralUtilities;
...
public class Log {
    ...
    public static void writeOperationStarted(String operation) {
        String msjLogger = String.format("----------Operation: %s ----------", operation);
        Log.LOGGER.info(msjLogger);
    }

    @SneakyThrows
    public static void writeOperationExeption(Exception exception) {
        Log.LOGGER.error("Operation failed: ".concat(exception.getMessage()), exception);
        throw exception;
    }

    public static void writeOperationCompleted() {
